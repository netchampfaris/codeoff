	...
	const d = new frappe.ui.Dialog({
		...
		primary_action(values) {
			const plan = matches.map((m) => ({
				...
				duration_seconds:
					values[`duration_r${m.round_number}_p${m.bracket_position}`] || defaultDuration,
			}))
			frm
				.call('create_bracket_from_plan', { plan: JSON.stringify(plan) })
				.then(() => {
					d.hide()
					frm.reload_doc()
				})
