<template>
  <div class="flex h-full flex-col bg-term-bg font-mono">
    ...
    <div class="flex-1 overflow-auto p-4">
      ...
      <h2
        class="mb-4 text-base font-bold uppercase tracking-wider text-term-green"
      >
        {{ problem.title }}
      </h2>
      <div
        class="prose prose-sm prose-invert max-w-none text-sm leading-relaxed text-green-200 [&_code]:font-mono [&_code]:text-term-green [&_h3]:uppercase [&_h3]:tracking-wider [&_h3]:text-green-400 [&_strong]:text-green-300"
      ...
      <div v-if="problem.constraints_text" class="mt-5">
        ...
        <div
          class="whitespace-pre-wrap border border-term-border bg-term-surface p-3 text-sm text-green-300"
        >
      ...
      <div v-if="problem.sample_test_cases?.length" class="mt-5">
        ...
        <div
          ...
          :key="i"
          class="mb-3 border border-term-border bg-term-surface p-3 text-sm"
        >
          <div class="mb-1">
            <span class="text-green-700">$ input:</span>
            <code class="ml-2 text-term-green">{{ tc.input }}</code>
          </div>
          <div>
