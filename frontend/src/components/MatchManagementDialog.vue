    ...
    >
      ...
      >
        <div>
          ...
          <div class="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs uppercase tracking-widest text-green-700">
      ...
      <div class="space-y-6 px-5 py-5">
        <div
          ...
          :class="feedbackTone === 'error' ? 'border-red-900 bg-red-950/30 text-red-300' : 'border-green-900 bg-green-950/20 text-green-300'"
        >
          {{ feedback }}
        </div>
...
function failAction(err: any) {
  pendingAction.value = null
  setFeedback(
    err?.messages?.[0]?.message || err?.message || 'Match action failed',
    'error',
  )
}
...
const judgeNowCall = useCall({
  ...
  onSuccess(data: any) {
    endAction(
      data?.status === 'Review' ? 'Match moved to review' : 'Match resolved',
    )
  },
  onError: failAction,
})
