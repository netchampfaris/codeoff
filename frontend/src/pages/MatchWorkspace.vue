...
// --- Resizable panels ---
const { problemPanelWidth, bottomPanelHeight, startDragH, startDragV } = useResizablePanels()
...
// --- Code draft ---
const { code, onCursorChange } = useCodeDraft(props.matchId, state, findMyPlayerId)
...
const bestScore = computed(() => {
	...
	return judged.reduce((best, s) => (s.passed_tests > best.passed_tests ? s : best), judged[0])
})
...
const submitCodeCall = useCall({
	...
	onError(err: any) {
		submitting.value = false
		submitError.value = err?.messages?.[0]?.message || err?.message || 'Submission failed'
	},
})
