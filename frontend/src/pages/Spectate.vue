<template>
			...
			<div v-else-if="state" class="relative flex h-full min-h-0 flex-col">
				...
				>
					<transition name="banner-fade">
						...
						>
							<div class="text-current/70 text-[10px] uppercase tracking-[0.35em]">
								{{ activeBanner.label }}
							</div>
							<div
				...
				<div class="hidden flex-1 flex-col overflow-hidden pb-16 md:flex">
					...
					<div class="grid grid-cols-3 border-b border-zinc-800 bg-zinc-900 px-6 py-2">
						<!-- Player 1 -->
						<div
	...
	<MatchManagementDialog v-model="showManageMatch" :match="state" @refreshed="reload" />
</template>

<script setup lang="ts">
import { ref, computed, watch, onMounted, onUnmounted } from 'vue'
import { useCall } from 'frappe-ui'
import { useMatchState, useMatchTimer, usePostDeadlineRefresh } from '@/data/match'
import { useLobbyPoll } from '@/data/useLobbyPoll'
import { useMatchPlayers } from '@/data/useMatchPlayers'
import { getSocket } from '@/data/socket'
import { getPredictionStats, getStoredPrediction, setStoredPrediction } from '@/data/predictions'
...
const { winnerName, player1Submissions, player2Submissions, bestScore1, bestScore2 } =
...
const predictionStatusLabel = computed(() => {
	if (!state.value || !selectedPlayerId.value || !selectedPlayerName.value) return ''
	if (state.value.status === 'Live') return `your pick: ${selectedPlayerName.value}`
	if (state.value.status === 'Review') return `your pick: ${selectedPlayerName.value} · pending`
	if (state.value.status === 'Finished') {
		if (!state.value.winner) return `your pick: ${selectedPlayerName.value} · draw`
...
watch(momentumSnapshot, (next, prev) => {
	...
	if (next.winner && next.winner !== prev.winner && next.winningReason === 'First Accepted') {
		pushMomentumBanner(
			'solve landed',
